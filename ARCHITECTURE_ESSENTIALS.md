...
- **Optimistic State & Tree Mutation Invariant:** Frontend mutations (`VANILLA:ADD`, `SCAFFOLD:TEXTURE_VARIATION`, `DELETE:TEXTURE_VARIATION`) perform immediate in-place store updates in `frontend/src/store/packStore.ts` before IPC responses arrive, preserving active workspace selections, scroll offsets, and expansion states without flickering. Handlers in `Views/MainWindow.xaml.cs` execute disk I/O and JSON rewriting via background tasks (`Task.Run`) before triggering asynchronous rescans, keeping the WebView2 UI thread unblocked.
- **Fallback Block & Workspace Tree Invariant:** In `Services/PackScanner.cs` (`BuildBlockWorkspaceTree`), declared aliases that are unassigned to any block in either user or vanilla `blocks.json` are elevated to dedicated fallback `BlockGroupNode`s using vanilla display names and face usages rather than collapsing into `(Uncategorized)`. Only true disk orphans (files on disk not declared in any JSON) surface under `(Uncategorized)` with `Orphan` status.
