...
- **Alias vs Block Add Invariant:** `VANILLA:ADD` (`Services/IpcContracts.cs` `VanillaAddPayload` + `Services/Ipc/Handlers/CatalogIpcHandlers.cs` + `frontend/src/components/catalog/CatalogDrawer.tsx`) must distinguish alias-only vs block adds via optional `alias` field — alias names can equal blockIds (`obsidian`) or differ (`glowing_obsidian` alias vs `glowingobsidian` block `vanilla_cache/blocks.json` / `terrain_texture.json`). Alias-only uses `JsonWriterService.AddVanillaBlockAlias` (terrain only). Declaration checks (`isAliasDeclaredInPack` in `CatalogDrawer.tsx`, `isDeclaredInTerrainTexture` in `BlockWorkspace.tsx`/`BlockEntryTree.tsx`) must use pack `aliases` (`status !== 'ORPHAN' && isUserDefined !== false`) independent of `block.isUserDefined`; otherwise vanilla fallback blocks with terrain alias (glowing case) incorrectly show as not-added/`vanilla fallback`. For shared aliases (one texture used by multiple blocks, e.g. `stone`), an orphan PNG must surface as fallback under *all* sharing blocks, and adding the alias once must mark it Added for all sharing blocks (alias is global in `terrain_texture.json`).
- **Optimistic State & Tree Mutation Invariant:** Frontend mutations (`VANILLA:ADD`, `SCAFFOLD:TEXTURE_VARIATION`, `DELETE:TEXTURE_VARIATION`) perform immediate in-place store updates via pure immutable functions in `frontend/src/store/mutations/workspaceTreeMutations.ts` before IPC responses arrive, preserving active workspace selections, scroll offsets, and expansion states without flickering. Handlers in `Services/Ipc/Handlers/` execute disk I/O and JSON rewriting via background tasks (`Task.Run`) before triggering asynchronous rescans, keeping the WebView2 UI thread unblocked.
- **Fallback Block & Workspace Tree Invariant:** In `Services/Scanning/WorkspaceTreeBuilder.Block.cs` (`BuildBlockWorkspaceTree`), declared aliases that are unassigned to any block in either user or vanilla `blocks.json` are elevated to dedicated fallback `BlockGroupNode`s using vanilla display names and face usages rather than collapsing into `(Uncategorized)`. Only true disk orphans (files on disk not declared in any JSON, resolved via `Services/Scanning/OrphanResolver.cs`) surface under `(Uncategorized)` with `Orphan` status.
...
| Pack scanning engine & pipeline orchestration | `Services/PackScanner.cs` delegating to `Services/Scanning/` domain parsers |
| Texture atlas (`terrain_texture.json`, `item_texture.json`) & flipbooks | `Services/Scanning/TextureAtlasParser.cs` |
| Block definitions (`blocks.json`), texture sets & face bindings | `Services/Scanning/BlockDefinitionParser.cs` |
| Client entity definitions & attachable model bindings | `Services/Scanning/EntityDefinitionParser.cs` |
| 3D Catalog, Block & Entity workspace tree construction | `Services/Scanning/WorkspaceTreeBuilder.cs`, `WorkspaceTreeBuilder.Block.cs`, `WorkspaceTreeBuilder.Entity.cs` |
| Disk file enumeration, orphan classification & companion linking | `Services/Scanning/OrphanResolver.cs` |
| Shared JSON parsing utilities & reader options | `Services/Scanning/ScanningJsonUtils.cs` |
| Pack archiving, extraction & zip stream safety | `Services/PackArchiveUtility.cs` |
| Vanilla assets, catalog manager, titlebar download notification | `Services/VanillaDataService.cs`, `Services/CatalogReferenceService.cs`, `frontend/src/components/titlebar/AssetDownloadNotification.tsx`, `frontend/src/components/catalog/ReferencePackManagerModal.tsx`, `frontend/src/components/catalog/CatalogDrawer.tsx` (toolbar: `CatalogToolbar.tsx`, card: `CatalogItemCard.tsx`, filter: `useCatalogFilter.ts`); always-visible catalog toggle FAB: `frontend/src/components/catalog/CatalogFab.tsx` |
| Drag-and-drop texture import & tile-to-tile copy | `TEXTURE:DROP_IMPORT`, `TEXTURE:COPY_FILE` IPC (`Services/IpcContracts.cs`, handled in `TextureIpcHandlers.cs`); frontend: `frontend/src/components/grid/PackGrid.tsx`, `frontend/src/components/workspace/WorkspaceTileCard.tsx`, `frontend/src/components/grid/TextureDropConfirm.tsx`, `frontend/src/components/catalog/CatalogDrawer.tsx`, `useIpc.dropImportTexture`, `useIpc.copyTextureFile` |
| Unified Workspace Shell chrome & responsiveness | `frontend/src/components/workspace/WorkspaceShell.tsx` and `WorkspaceShell.module.css` |
...
| Texture grid and shared morph preview | `frontend/src/components/grid/PackGrid.tsx`, `TileHoverMorphPortal.tsx` (coordinates: `useMorphCoordinates.ts`, action bar: `MorphActionsBar.tsx`) |
...
| Pack export (`.mcpack` archive) | `Services/PackExportService.cs`, `Services/PackArchiveUtility.cs`, `frontend/src/components/menus/MenuBar.tsx` (`handleExportPack`), `PACK:EXPORT` IPC |
| Companion atlas textures (`_atlas`) & interactive animations | `Services/Scanning/OrphanResolver.cs` (atlas linking & orphan suppression), `Services/PackArchiveUtility.cs` (`ExtractCompanionAtlasIfExists`), `frontend/src/components/common/FlipbookThumbnail.tsx` (real-time clock & mouse compass) |
